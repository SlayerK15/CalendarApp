...
The application and deployment configuration are implemented. Production deployment and real Google API acceptance checks require deployment-account access and OAuth credentials. The supplied spreadsheet returned HTTP 401 to an anonymous export, so its actual format is not yet verified. The current parser supports the documented [row contract](docs/SHEET_FORMAT.md); an Excel/CSV export is needed if the real timetable uses a different layout. Landing-page sample classes are explicitly illustrative, never shown as a user's real timetable.
