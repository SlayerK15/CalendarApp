...
export type Dashboard = {timetables: {label: string; url: string; enabled: boolean}[]; programme: string; section: string; active: boolean; pending: boolean; sync_interval_seconds: number; sync_on_change: boolean; sync_in_progress: boolean; calendar_id: string | null; last_synced_at: string | null; last_error: string | null; event_count: number; watch_active: boolean; events: TimetableEvent[]; runs: {status: string; message: string; at: string}[]};
